...
public static class SetupMonolithGlow {
 public static object Main(){
 if(Application.isPlaying)return "PLAYING";
 var station=UnityEngine.Object.FindFirstObjectByType<EnergyMonolyth>();
 ...
 if(!child){var go=new GameObject("Night Energy Glow");Undo.RegisterCreatedObjectUndo(go,"Add monolith night glow");go.transform.SetParent(source.transform,false);child=go.transform;}
 ...
 var lamp=station.transform.Find("Night Light");if(!lamp){var go=new GameObject("Night Light");Undo.RegisterCreatedObjectUndo(go,"Add monolith night light");go.transform.SetParent(station.transform,false);lamp=go.transform;}
 ...
 controller.sky=sky;controller.source=source;controller.glow=glow;controller.localLight=light;
 ...
 if(Vector3.Distance(light.transform.position,source.bounds.center)>.001f)throw new Exception("Light position incorrect");
