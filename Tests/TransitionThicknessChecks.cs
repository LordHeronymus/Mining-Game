 ...
 public static object Main()
 {
  ...
  try
  {
   ...
   foreach(int width in new[]{5,15,30})
   {
    ...
    var sampler=new MapGenerationSampler(registry,map.seed,map.mapHeight,map.layers,map.oreDensityCurve,map.oreDensityMultiplierPercent,width,map.oreTransitionCurve,map.oreTransitionDepth, map.oreVeinSizeCurve, map.surfaceOreRampDepth, map.surfaceOreRampCurve);
